// RadioModule.cpp — Fixed compilation errors
#include <Arduino.h>
#include <SPI.h>
...
// --------- Constants ---------
    ...
    // --------- File-local Helpers ---------
    static bool waitForModeReady(RFM69 & radio, uint32_t timeoutMs = 50, const char* context = "unknown") {
        const uint32_t start = millis();
        while (!(radio.readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY)) {
            if (millis() - start > timeoutMs) {
                RDBG_PRINTF("ModeReady timeout in %s after %lu ms\n", context, (unsigned long)timeoutMs);
                return false;
            }
    ...
    bool RadioModule::receive(uint8_t * data, size_t & len) {
        // Add interrupt safety for receive operations
        noInterrupts();
    ...
    bool RadioModule::checkRadioSettings(uint8_t expectedNodeId, uint8_t expectedNetworkId, uint32_t expectedFreqHz) {
        ...
        if (abs((int32_t)(actualFreqHz - expectedFreqHz)) > 1000) {
    ...
    bool RadioModule::setFrequencyBlocking(uint32_t freqHz) {
        ...
        if (!waitForModeReady(_radio, 50, "setFrequency_standby")) return false;
        ...
        if (!waitForModeReady(_radio, 50, "setFrequency_synth") || !waitForPllLock(_radio)) return false;
        ...
        const uint8_t * msg, uint8_t len)
    {
        ...
        // 1) Switch frequency and wait STANDBY
        if (!setFrequencyBlocking(destFreqHz)) {
            RDBG_PRINTLN("Failed to set destination frequency");
            return false;
        }
        _radio.setMode(RF69_MODE_STANDBY);
        if (!waitForModeReady(_radio, 50, "sendWithReturn_standby")) {
            RDBG_PRINTLN("Standby mode timeout");
            return false;
        }

        // 2) Send packet - library handles TX mode internally
        _radio.send(destNode, msg, len, false);

        // 3) Small delay to ensure transmission completion before frequency change
        delay(5); // Allow transmission to complete

        // 4) Switch to return frequency
        if (!setFrequencyBlocking(returnFreqHz)) {
            RDBG_PRINTLN("Failed to set return frequency");
            return false;
        }
        _radio.setMode(RF69_MODE_RX);
        if (!waitForModeReady(_radio, 50, "sendWithReturn_rx")) {
    ...
    void RadioModule::sendRTCM(const uint8_t* data, size_t len) {
        yield(); // Yield BEFORE starting transmission

        // Extract RTCM message type BEFORE transmission
        if (len >= 6 && data[0] == 0xD3) {
            uint16_t messageType = (data[3] << 4) | (data[4] >> 4);
            typeCounter.recordMessage(messageType);
        }
        ...
        utilizationMonitor.recordTransmission(len, wasFragmented);
    }
    ...
    void RadioModule::enterSleepMode() {
        _radio.setMode(RF69_MODE_SLEEP);
        waitForModeReady(_radio, 100, "sleep");
    }

    void RadioModule::wakeFromSleep() {
        _radio.setMode(RF69_MODE_STANDBY);
        waitForModeReady(_radio, 100, "wake");
    }
    ...
    void RadioModule::RTCM_Fragmenter::sendFragmented(RFM69 & radio, uint8_t destId,
        ...
        for (uint8_t i = 0; i < totalChunks; ++i) {
            ...
            // FIX: Cast types properly for min/max functions
            const uint8_t adaptiveDelay = max(5U, min(50U, static_cast<unsigned int>(chunkLen) / 2));
            delay(adaptiveDelay);
        }
    }
